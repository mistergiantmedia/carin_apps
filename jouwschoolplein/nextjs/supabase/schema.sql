...
-- Schoolplein is one open community. Activities are not restricted by school group.
create type role_enum as enum ('PARENT','STUDENT','FORMER_STUDENT','VOLUNTEER','ORGANIZER','SCHOOL_ADMIN');
...
create table if not exists reports (
  ...
  message_id uuid references activity_messages(id),
...
create table if not exists community_settings (
  ...
  monthly_activity_limit int default 15,
  former_student_max_age int default 18
);
...
create policy "members can join activities" on activity_participants for insert with check (user_id=auth.uid() and exists(select 1 from activities a join school_memberships m on m.school_id=a.school_id where a.id=activity_participants.activity_id and m.user_id=auth.uid()));
...
create policy "members can post messages" on activity_messages for insert with check (user_id=auth.uid() and exists(select 1 from activities a join school_memberships m on m.school_id=a.school_id and m.user_id=auth.uid() where a.id=activity_messages.activity_id));
create policy "members can view community settings" on community_settings for select using (exists(select 1 from school_memberships m where m.school_id=community_settings.school_id and m.user_id=auth.uid()));
