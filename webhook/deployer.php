...
// if none of the above match, but a token exists, exit
    ...
    if ($json["ref"] === BRANCH) {
        ...
        if (file_exists($DIR . ".git") && is_dir($DIR)) {
            ...
            // Add the safe directory configuration before pulling
            exec(GIT . " config --global --add safe.directory " . escapeshellarg($DIR) . " 2>&1", $output, $exit);
            
            
            
            
            // Add this before the pull command
            exec(GIT . " reset --hard HEAD 2>&1", $outputReset, $exitReset);
            $outputReset = (!empty($outputReset) ? implode("\n", $outputReset) : "[no output]") . "\n";
            if ($exitReset !== 0) {
                http_response_code(500);
                $outputReset = "=== ERROR: Reset failed using GIT `" . GIT . "` ===\n" . $outputReset;
            }
            fputs($file, $outputReset);
            echo $outputReset;
